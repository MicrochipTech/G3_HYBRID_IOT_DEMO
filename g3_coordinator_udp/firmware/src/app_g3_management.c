    ...
    while (checkAllPanId == true)
    {
        ...
        for (uint8_t i = 0; i < app_g3_managementData.numNetworksFound; i++)
        {
            if (app_g3_managementPanIdList[i] == app_g3_managementData.panId)
            {
                /* Change PAN ID */
                app_g3_managementData.panId += 1;
                app_g3_managementData.panId = APP_G3_MANAGEMENT_PAN_ID + (~APP_G3_MANAGEMENT_PAN_ID_MASK & app_g3_managementData.panId);
                checkAllPanId = true;
                break;
            }
        }
...
int flag_received_alarm = 0;
int led_alarm_times = 0;
...
void APP_G3_MANAGEMENT_Tasks ( void )
{
    ...
    if (app_g3_managementData.timerLedExpired == true)
    {
        ...
        /* Handle RGB Red Alarm */
        if (led_alarm_times)
        {
            led_alarm_times--;
            if (led_alarm_times == 0)
                RGB_LED_RED_Off();
        }
    }
    ...
    switch ( app_g3_managementData.state )
    {
        ...
        {
            /* Nothing to do */
            if (flag_received_alarm)
            {
                led_alarm_times = 4;
                RGB_LED_RED_On();
                APP_USI_DEVICE_Send_Emergency();
                SYS_DEBUG_MESSAGE(SYS_ERROR_DEBUG, "APP_G3_MANAGEMENT: USI Emergency Notification sent\r\n");
                flag_received_alarm = 0;
            }
            break;
        }            
        ...
        case APP_G3_MANAGEMENT_STATE_ERROR:
        {
            ...
            SYS_DEBUG_MESSAGE(SYS_ERROR_INFO, "APP_G3_MANAGEMENT: STATE_ERROR\r\n");
            break;
        }

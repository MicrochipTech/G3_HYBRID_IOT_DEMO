...
static void _LBP_COORD_JoinRequestIndication(uint8_t* pLbdAddress)
{
    ...
    if (app_eap_serverData.rekey == false)
    {
        ...
        if (blacklisted == false)
        {
            ...
            {
                //assignedAddress = APP_EAP_SERVER_INITIAL_SHORT_ADDRESS + app_eap_serverData.numShortAddrAssigned++;
                assignedAddress = pLbdAddress[7]; // Fixed short address by extended address
...
{   
    ...
    if (memcmp(app_eap_serverExtAddrList[index].value, app_eap_serverNullAddress.value, ADP_ADDRESS_64BITS) == 0)
    {
        /* EUI64 Not found in the list - Include into the list */
...
static void _LBP_COORD_LeaveIndication(uint16_t networkAddress)
{
            ...
            (networkAddress < (APP_EAP_SERVER_INITIAL_SHORT_ADDRESS + APP_EAP_SERVER_MAX_DEVICES)))
    {
        ...
        APP_COORDINATOR_deviceInfoRefresh(index, &app_eap_serverExtAddrList[index].value, networkAddress, false);
...
uint16_t APP_EAP_SERVER_GetDeviceAddress(uint16_t index, uint8_t* pEUI64)
{
    ...
    {
        /* Check not null address */
        if (memcmp(app_eap_serverExtAddrList[index].value,
                app_eap_serverNullAddress.value, ADP_ADDRESS_64BITS) != 0)
        {
    ...
    if (memcmp(app_eap_serverExtAddrList[index].value, app_eap_serverNullAddress.value, ADP_ADDRESS_64BITS) != 0)
    {
        memcpy(pEUI64, app_eap_serverExtAddrList[index].value, ADP_ADDRESS_64BITS);
        return index;
    }
